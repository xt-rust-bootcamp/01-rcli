...
pub enum TextSubCommand {
    ...
    #[command(
        about = "Generate a random blake3 key or ed25519 key pair or ChaCha20Poly1305 key&nonce"
    )]
...
impl CmdExector for TextVerifyOpts {
    async fn execute(self) -> anyhow::Result<()> {
        let mut reader = get_reader(&self.input)?;
        let key = get_content(&self.key)?;
        let decode = URL_SAFE_NO_PAD.decode(&self.sig)?;
        let verified = process_text_verify(&mut reader, &key, &decode, self.format)?;
        if verified {
